...
2. **Download built-in defaults** — same company picker, but writes the selected file's JSON to a file the user picks (for editing/distribution as a starting template), **with `loginQrKey` stripped out** — see §B.6.6.
...
`getExtSystemDefaultsJsonForExport(fileName)` backs "download as file": it parses the bundled asset, removes `loginQrKey`, and re-serializes pretty-printed (via a separate `exportGson`; `JsonObject` preserves the other fields and their order, including ones no DTO knows about, such as `companyName`). It then re-reads the asset's own key and refuses to return anything if that string survived into the output — a renamed field or a stray second copy fails the export (the caller's existing save-failed toast) rather than leaking the key. The field name lives in the file-level `LOGIN_QR_KEY_FIELD` constant, which mirrors the DTO property by hand; that guard exists because the two can drift.
...
**The key never leaves the app through "Download built-in defaults."** An exported file lands in shared storage and then travels by mail and chat; that is precisely how the one secret protecting printed codes ends up somewhere it cannot be recalled. `getExtSystemDefaultsJsonForExport` strips the field and verifies the key is gone before writing (§B.6.6). The counterpart rule is on import: a configuration with no key leaves the device's existing key alone, so a stripped file remains a usable template and does not quietly turn QR sign-in off. Devices get the key from the bundled assets or from a file prepared deliberately for a rotation — never from a copy someone exported. Note this narrows the *export* path only; the key is still in the APK and in git, so it stays obfuscation-grade (see below).
...
- **Adding a new document type**: add an enum value to `DocumentType` (Models.kt) with a `key`/`display`, plus `retailLocation` if it shares a `Document_Type` code with another type and `defaultFilterMode` if location isn't the right scope; add its code to the four bundled `ext_system_defaults_*.json`; add a localized name in all four `strings.xml`. It will automatically appear in `MainMenuScreen`'s type list and all downstream screens, since they iterate `DocumentType.entries` — but note the compiler will also stop you in seven exhaustive `when` blocks that must each get a branch: six duplicated `localizedDisplay()` helpers (one per screen) and `MainMenuScreen`'s icon mapping. That the compiler catches every one of them is the reason the duplication is survivable.
