...
3. `AppViewModel.saveCredentials(username, password)` → `extSystemCredentialStore.save(...)`, refreshes the `_credentials` `StateFlow`. **Returns `false` and stores nothing** when `UserProfileStore.normalise(username)` is not the signed-in profile's id: the sheet is reachable from inside a session, so someone handed an already-signed-in device could type their own ERP account into it and have it filed under the other person's profile — every row uploaded afterwards then travelled on the wrong session while still carrying the signed-in operator's name. `testExtSystemConnection` turns that refusal into a `Failure` (`R.string.signin_wrong_operator`), so all five `LoginSheet` call sites surface it inline. Testing with an account other than your own therefore means signing in as that account first.
